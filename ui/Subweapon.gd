...
func error_no_subweapon() -> void:
	tween.stop_all()
	tween.interpolate_property(current, "self_modulate", UI.error_color, Color("#808080"), 1.0, Tween.TRANS_CUBIC, Tween.EASE_OUT)
	tween.start()
...
func update_subweapon_icon() -> void:
	...
	if Player.current_subweapon != '':
		current.self_modulate = UI.jewel_color
	else:
		current.self_modulate = Color("#808080")
	
	for sprite in current.get_children():
		sprite.visible = (sprite.name == Player.current_subweapon)
