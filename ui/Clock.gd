...
func _process(_t:float) -> void:
	hour_hand.rotation = Timekeeper.time_of_day / 720.0 * TAU
	minute_hand.rotation = fmod(Timekeeper.time_of_day, 60.0) * TAU / 60.0
	
	# maybe this should use a fade with 2 sprites instead of animatedsprite.
	if Timekeeper.time_of_day > 360.0 and Timekeeper.time_of_day < 1080.0:
		sunmoon.frame = 0
	else:
		sunmoon.frame = 1
