...
var target_pos:Vector3 # target position, world space
...
func _ready() -> void:
	...
	shape.collision_mask = Layers.solid | Layers.slippery
...
func update_target() -> void:
	...
	target_pos = Game.player.global_transform.origin + player_height_offset + pan_pos
...
func _physics_process(_t:float) -> void:
	...
	if Input.is_action_just_pressed('target'):
		# find targets
		# hmmm...
		# maybe a giant area that if a targetable object enters it gets added to a list of objects
		# then go through the list to find the most applicable target
		# if no target can be found, reset camera...
...
func update_position(new_pos:Vector3) -> void:
	...
	shape.transform = Transform(Basis(), target_pos) # start at the player,
	...
	if result[0] > 0: # result[0] is how much to lerp
		new_pos = target_pos.linear_interpolate(new_pos + target_pos, result[0]) # now we have final position
		look_at_from_position(new_pos, target_pos, Vector3.UP) # look at player from final position
	current_pos = (self.global_transform.origin - target_pos).normalized() # update current position
